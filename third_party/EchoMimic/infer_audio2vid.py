...
def main():
    ...
    vae.enable_slicing()  # 将视频帧逐个解码，而不是同时解码所有帧。用时间换显存！
    vae.enable_tiling()   # 将单张图片切分成块解码。
    ...
    face_detector = MTCNN(image_size=320, margin=0, min_face_size=20, thresholds=[
    ...
    for ref_image_path in config["test_cases"].keys():
        for audio_path in config["test_cases"][ref_image_path]:
            ...
            if select_bbox is None:
                ...
                crop_rect = [max(0, cb - c_pad_crop), max(0, rb - r_pad_crop), min(
                    ce + c_pad_crop, face_img.shape[1]), min(re + r_pad_crop, face_img.shape[0])]
                print(crop_rect)
            ...
            face_mask_tensor = torch.Tensor(face_mask).to(
                dtype=weight_dtype, device="cuda").unsqueeze(0).unsqueeze(0).unsqueeze(0) / 255.0
            ...
            video_clip.write_videofile(
                f"{save_dir}/{ref_name}_{audio_name}_{args.H}x{args.W}_{int(args.cfg)}_{time_str}_withaudio.mp4", codec="libx264", audio_codec="aac")
            print(
