...
class VisemeEncoder(nn.Module):
    ...
    def __init__(self, num_visemes, d_model=256, nhead=8, num_layers=4, out_dim=None, fau_dim=16):
        ...
        self.output_projection = nn.Linear(
            d_model, out_dim) if out_dim is not None else nn.Identity()
    ...
    def forward(self, viseme_ids, target_frames_len=None, viseme_padding_mask=None, fau_signals=None):
        ...
        if isinstance(encoder, nn.TransformerEncoder):
            memory = encoder(x, src_key_padding_mask=viseme_padding_mask)
        else:
            memory = encoder(x)

        # 2. Generate frame queries (B, T, d_model)
        frame_positions = torch.arange(
            T, device=viseme_ids.device).unsqueeze(0).expand(B, T)
        frame_queries = self.frame_query_embed(
            frame_positions) * math.sqrt(self.d_model)
        ...
        if isinstance(decoder, nn.TransformerDecoder):
            out = decoder(tgt=frame_queries, memory=memory,
                          memory_key_padding_mask=viseme_padding_mask)
        else:
            out = decoder(memory, target_frames_len=T)
